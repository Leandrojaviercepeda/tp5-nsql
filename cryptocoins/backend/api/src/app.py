from db.mongo import db_cryptocoins
...
def fetch_cryptocoins():
    ...
    try:
        ...
        db_cryptocoins.encryptedcoins.drop()
        # Planchamos las colecciones de monedas desencriptadas
        db_cryptocoins.decryptedcoins.drop()
        # Añadimos los datos nuevamente
        db_cryptocoins.encryptedcoins.insert_many(listhings_encrypted)
        # Añadimos los datos nuevamente
        db_cryptocoins.decryptedcoins.insert_many(listhings)
        return 'OK'
    except:
        raise
...
def get_cryptocoins():
    ...
    try:
        ...
        cryptocoins_encrypted = list(db_cryptocoins.encryptedcoins.find({},{"_id":0}).limit(500))
        # Buscamos los datos en la base de datos desencriptada y lo pasamos a una lista de python
        cryptocoins_decrypted = list(db_cryptocoins.decryptedcoins.find({},{"_id":0}).limit(500))
...
def cryptocoins(rank):
    try:
        if request.method == 'DELETE':
            ...
            DeleteResultE = db_cryptocoins.encryptedcoins.delete_one({"cmc_rank": rank})
            # Borramos el dato de la coleccion de monedas desencriptadas
            DeleteResultD = db_cryptocoins.decryptedcoins.delete_one({"cmc_rank": rank})
        ...
        if request.method == 'GET':
            # Buscamos los datos en la base de datos encriptada y lo pasamos a una lista de python
            cryptocoin_encrypted = list(db_cryptocoins.encryptedcoins.find({"cmc_rank": rank},{"_id":0}))
            # Buscamos los datos en la base de datos desencriptada y lo pasamos a una lista de python
            cryptocoin_decrypted = list(db_cryptocoins.decryptedcoins.find({"cmc_rank": rank},{"_id":0}))
            # Encriptamos los datos de la base de datos sin encriptar
            ...
            if(cryptocoin_encrypted==listhing_encrypted):
                ...
                fetch_cryptocoins()
                # Si fueron alterados
                return dict(db_cryptocoins.decryptedcoins.find({"cmc_rank": rank},{"_id":0}))
    except:
        raise
...
def get_cryptocoins_top_20():
    ...
    try:
        # Buscamos los datos en la base de datos encriptada y lo pasamos a una lista de python
        cryptocoins_encrypted = list(db_cryptocoins.encryptedcoins.find({},{"_id":0}).limit(20))
        # Buscamos los datos en la base de datos desencriptada y lo pasamos a una lista de python
        cryptocoins_decrypted = list(db_cryptocoins.decryptedcoins.find({},{"_id":0}).limit(20))
        # Encriptamos los datos de la base de datos sin encriptar
        listhings_encrypted = [{"cmc_rank":thing["cmc_rank"],"data": hashdata(thing)} for thing in cryptocoins_decrypted]
        # Comparamos los datos para saber si fueron alterados        
        if(cryptocoins_encrypted==listhings_encrypted):
...
def get_cryptocoins_top_5():
    ...
    try:
        # Buscamos los datos en la base de datos encriptada y lo pasamos a una lista de python
        cryptocoins_encrypted = list(db_cryptocoins.encryptedcoins.find({},{"_id":0}).limit(5))
        # Buscamos los datos en la base de datos desencriptada y lo pasamos a una lista de python
        cryptocoins_decrypted = list(db_cryptocoins.decryptedcoins.find({},{"_id":0}).limit(5))
        # Encriptamos los datos de la base de datos sin encriptar
        listhings_encrypted = [{"cmc_rank":thing["cmc_rank"],"data": hashdata(thing)} for thing in cryptocoins_decrypted]
        # Comparamos los datos para saber si fueron alterados        
        if(cryptocoins_encrypted==listhings_encrypted):
